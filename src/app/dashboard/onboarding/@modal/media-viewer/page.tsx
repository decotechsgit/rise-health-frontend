...
const Page = async ({ searchParams }: { searchParams: Promise<PageSearchParams> }) => {

  const { id, title } = await searchParams;

  const progress = await onboardingService.getOnboardingProgress();

  const mediaData = progress?.progress?.uploads?.receive_file_upload?.[id] || [];

  return (
    <section className="absolute top-0 right-0 w-full h-full z-50 bg-transparent">
      {mediaData.length > 0 ? (
