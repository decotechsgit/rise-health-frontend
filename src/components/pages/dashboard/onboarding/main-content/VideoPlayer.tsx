...
const VideoPlayer = ({ videoUrl }: { videoUrl?: string | null }) => {
  ...
  return (
    <>
      ...
      >
        <div className="overflow-hidden rounded aspect-video bg-[#F8F8F8] flex items-center justify-center">
          {videoUrl ? (
      ...
      {hasWindow && isModalOpen && videoUrl && (
        ...
        >
          <div
            className="relative w-[90%] md:w-[80%] lg:w-[70%] max-w-6xl"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={closeModal}
              className="absolute -top-3 -right-3 text-white bg-black/50 rounded-full p-1.5 z-50 transition-transform hover:scale-110"
